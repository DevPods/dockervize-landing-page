...
docker tag dockervize/dev:latest 114736952595.dkr.ecr.us-east-1.amazonaws.com/dockervize-homepage:$TRAVIS_COMMIT
# Push built image to ECS
docker push 114736952595.dkr.ecr.us-east-1.amazonaws.com/dockervize-homepage:$TRAVIS_COMMIT
...
aws elasticbeanstalk create-application-version --application-name dockervizehomepage --version-label $TRAVIS_COMMIT --source-bundle S3Bucket=$EB_BUCKET,S3Key=dockerdeploy.zip
# Update environment to use new version number
aws elasticbeanstalk update-environment --environment-name Dockervizehomepage-env-1 --version-label $TRAVIS_COMMIT